// AUL/UUL disindaki Validasyon Takip (VALIDATION_FLOW) noktalarini yeniden hesaplar.
// Kurallar (baseline fix ile ayni mantik):
//  - source='VALIDATION_FLOW' ve recovery AUL/UUL disinda olan noktalar hedef
...
//  - yeni recovery, eski recovery'nin Xort tarafiyla AYNI tarafta kalir
//  - audit log'a HICBIR sey eklenmez
//  - VALIDATION_BASELINE ve MANUAL'a dokunulmaz
...
try {
  const res = await pool.query(`
    ...
    WHERE p.source = 'VALIDATION_FLOW'
  ...
  console.log(`Hedef Validasyon Takip nokta sayisi: ${res.rowCount}\n`);
  if (res.rowCount === 0) {
  ...
  console.log(`\nGuncellenen Takip nokta: ${updated}`);
} catch (err) {
