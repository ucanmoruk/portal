      ...
      <div className={styles.tableCard}>
        <div className={styles.tableWrapper}>
          <table className={`${styles.table} ${kys.acceptanceTable}`}>
            <thead><tr><th>Kabul tarihi</th><th>Kalem</th><th>Gelen miktar</th>{detail.satinAlmaYetkisi && <th>Tedarikçi</th>}<th>Değerlendiren</th><th>Genel değerlendirme</th><th>Belge / Düzeltme</th></tr></thead>
            <tbody>
              {detail.kabuller.length === 0 ? <tr><td colSpan={detail.satinAlmaYetkisi ? 7 : 6}><div className={styles.empty}>Kabul kaydı yok.</div></td></tr> : detail.kabuller.map(k => {
                const item = detail.kalemler.find(i => i.id === k.kalemId);
                return <tr key={k.id}><td>{dateFmt(k.kabulTarihi)}</td><td>{item?.malzemeAdi || k.kalemId}</td><td>{k.gelenMiktar} {item?.birim}</td>{detail.satinAlmaYetkisi && <td>{k.tedarikci || "-"}</td>}<td>{k.degerlendirenAd || "-"}</td><td>{k.genelDegerlendirme || "-"}</td><td>{detail.belgeler?.filter(b=>Number(b.KabulID)===k.id).map(b=><a key={b.ID} href={`/api/kys/talepler/${id}/belgeler/${b.ID}`} target="_blank" rel="noopener noreferrer" style={{display:"block"}}>{b.DosyaAdi}</a>)}<div className={styles.actionBtns}><button className={`${styles.editBtn} ${kys.iconButton}`} title="Düzelt" aria-label={`${item?.malzemeAdi || k.kalemId} kabulünü düzelt`} onClick={()=>openCorrect(k)}><Pencil size={16} aria-hidden="true" /></button><button disabled={saving} className={`${styles.deleteBtn} ${kys.iconButton}`} title="Kabulü sil" aria-label={`${item?.malzemeAdi || k.kalemId} kabulünü sil`} onClick={() => deleteAcceptance(k)}><Trash2 size={15} aria-hidden="true" /></button></div></td></tr>;
              })}
            </tbody>
          </table>
        </div>
