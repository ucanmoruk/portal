...
export default function RaporTakipTable({ fixedRaporTuru }: { fixedRaporTuru?: string } = {}) {
  ...
  const toggleRow = async (row: RaporRow) => {
    if (isUgdrFormat(row.RaporFormati)) {
      router.push(`/laboratuvar/rapor-takip/ugdr/${row.RaporNo}?nkrId=${row.NkrID}`);
      return;
    }
              ...
              >
                ...
                >
                  {isUgdrFormat(row.RaporFormati) ? (
                    <button
                      type="button"
                      onClick={() => window.open(`/laboratuvar/rapor-takip/ugdr/${row.RaporNo}?nkrId=${row.NkrID}`, "_blank", "noopener,noreferrer")}
                      style={{
                        border: "none",
