...
export default function UrunFormClient({ editId, source = 'ugd', returnHref = "/ugd/urun-listesi", displayLabel }: UrunFormClientProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([data, formulData]) => {
        if (data) {
          if (isLabSource && data.FirmaID) {
            setLookups(prev => {
              const firmaId = String(data.FirmaID);
              ...
              return {
                ...
                firmalar: [
                  ...prev.firmalar,
                  { ID: data.FirmaID, Ad: data.FirmaAd || `Firma #${firmaId}` },
                ],
              };
            });
          }
  ...
  useEffect(() => {
    ...
    fetch(`/api/numune-form/${editId}`)
      .then(r => r.ok ? r.json() : null)
      .then(payload => {
        if (!payload?.nkr) return;
        const nkr = payload.nkr;
        const detay = payload.detay || {};

        const fallbackMiktar = detay.Miktar != null
          ? `${detay.Miktar}${detay.Birim ? ` ${detay.Birim}` : ""}`
          : nkr.TesteMiktar != null
        ...
        setForm(prev => ({
          ...
          UrunEn: prev.UrunEn || nkr.Numune_Adi_En || prev.UrunEn,
        }));
      })
      .catch(() => {});
  }, [editId, isLabSource]);
  ...
  const saveFormulToDB = async (urunId: string, rows: MatchedIngredient[]) => {
    try {
      const res = await fetch(isLabSource ? `/api/laboratuvar/ugdr/${urunId}/formul` : "/api/urunler/formul", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(isLabSource ? { rows } : { urunId: Number(urunId), rows }),
      });
      if (!res.ok) {
  ...
  const handleReportDownload = async (format: "preview" | "word" | "pdf") => {
    ...
    try {
      ...
      // Dosya adı:
      //   • TR PDF → UGD_<Ürün Adı>.pdf
      //   • EN PDF → CPSR-<Ürün Adı>.pdf  (Cosmetic Product Safety Report)
      //   • TR/EN DOCX için aynı kuralı kullan (eski "UGD_Rapor_<RaporNo>" yerine ürün adı)
      const safeName = String(form.Urun || form.RaporNo || "rapor")
        .trim()
        .replace(/[\\/:*?"<>|]+/g, "_")     // Windows yasak karakterler
        .replace(/\s+/g, "_")
        .slice(0, 80) || "rapor";
          ...
          const savedSections = Array.isArray(saved.item.sections)
            ? saved.item.sections.map((section: any) => ({
                ...
                html: String(section?.html || "").trim()
                  ? refreshUgdReportHtml(reportLang === "cpsr" ? normalizeSavedCpsrHtml(section.html) : section.html, html, reportLang === "cpsr")
                  : "",
              }))
            : [];
    ...
    <div className={styles.page}>
      ...
      {savedOk && (
        <div style={{ marginBottom: 20, padding: '12px 18px', background: '#e6f4ea', border: '1px solid #a8d5b5', borderRadius: 8, fontSize: '0.85rem', color: '#1a7340', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button type="button" onClick={() => { router.push(returnHref); router.refresh(); }} style={{ background: 'none', border: '1px solid #1a7340', borderRadius: 6, padding: '4px 12px', fontSize: '0.8rem', color: '#1a7340', cursor: 'pointer', fontWeight: 600 }}>
            Listeye Dön
          </button>
        </div>
      )}
      ...
      {activeTab === 'genel' && (
        <SectionCard title="Genel Bilgiler">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            {isLabSource && (
              <div className={styles.formGroup}>
                <label>Ürün Adı (Eng)</label>
                <input value={form.UrunEn} onChange={e => setForm({ ...form, UrunEn: e.target.value })} />
              </div>
            )}
