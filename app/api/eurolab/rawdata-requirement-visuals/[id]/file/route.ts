  ...
  try {
    ...
    return new Response(new Uint8Array(row.file_data), {
      headers: {
        ...
        "Content-Disposition": `inline; filename="${encodeURIComponent(row.file_name || "gereklilik-kontrol-gorseli.pdf")}"`,
        "Cache-Control": "private, max-age=300",
      },
    });
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : "Gereklilik görseli açılamadı.";
