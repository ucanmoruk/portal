...
import { laboratoryMailBrand } from "@/lib/laboratuvarMailTemplate";
...
export async function POST(request: Request) {
  ...
  const brand = laboratoryMailBrand(cfg);
  if (body.action === "preview") return Response.json({ ...data, brand, mesaj: KABUL_MAIL_MESAJ, html: buildKabulMail(data, KABUL_MAIL_MESAJ, brand, "/unique-logo.png").html });
  ...
  try {
    ...
    try {
      const content = buildKabulMail(data, mesaj, brand);
      const logoPath = path.join(process.cwd(), "public", "unique-logo.png");
      const info = await transporter.sendMail({ from: setting("MAIL_FROM", user).trim(), to, cc, subject: konu, ...content, attachments: existsSync(logoPath) ? [{ filename: "unique-logo.png", path: logoPath, cid: "unique-logo" }] : [] });
