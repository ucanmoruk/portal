...
function relatedFormats(format: string): string[] {
  const base = baseReportFormat(format);
  const english = englishReportFormat(base);
  return [...new Set([base, english].filter(Boolean) as string[])];
}
...
export async function POST(
  ...
  try {
    ...
    const formats = relatedFormats(format);
    const onayRows: Array<{ ID: number; RaporFormati: string; DisRaporKodu: string | null }> = [];
    for (const fmt of formats) {
      const onayRes = await pool.request()
        .input("nkrId", nkrIdNum).input("format", fmt)
        .query(`
          SELECT TOP 1 ID, RaporFormati, DisRaporKodu FROM NKR_RaporOnay
          WHERE NkrID = @nkrId
            AND UPPER(REPLACE(RaporFormati, N'Ü', N'U')) = UPPER(REPLACE(@format, N'Ü', N'U'))
          ORDER BY ID DESC
        `);
      const row = onayRes.recordset[0];
      if (row) onayRows.push(row);
    }
    const onay = onayRows[0];
    if (!onayRows.length) {
    ...
    if (!aciklama) {
      ...
      if (present.has("nkr_log")) {
        await pool.request()
          ...
          .input("Aciklama", `${formats.join(", ")} formatları revizyon numarası artırılmadan onay alanına geri alındı.`.slice(0, 900))
          .query(
    ...
    if (present.has("nkr_log")) {
      await pool.request()
        ...
        .input("Aciklama", `${formats.join(", ")} formatları revize edildi (Rev.${eskiRev} → Rev.${yeniRev}). ${aciklama}`.slice(0, 900))
        .query(
          `INSERT INTO NKR_Log (NKRID, KullaniciID, Eylem, Aciklama, Tarih)
           VALUES (@NKRID, @KullaniciID, @Eylem, @Aciklama, CURRENT_TIMESTAMP)`
        );
